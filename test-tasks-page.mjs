...
async function testTasksPage() {
  ...
  try {
    ...
    if (stats > 0) {
      ...
      const totalTasks = await page.locator('text=Total Tasks').locator('..').locator('p.text-3xl').textContent();
      const todoCount = await page.locator('text=To Do').locator('..').locator('p.text-3xl').textContent();
      const inProgressCount = await page.locator('text=In Progress').locator('..').locator('p.text-3xl').textContent();
      const overdueCount = await page.locator('text=Overdue').locator('..').locator('p.text-3xl').textContent();
