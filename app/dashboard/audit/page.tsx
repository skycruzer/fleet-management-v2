...
export default async function AuditPage({ searchParams }: AuditPageProps) {
  ...
  if (
    searchParams.sortBy &&
    ['created_at', 'user_email', 'table_name', 'action'].includes(searchParams.sortBy)
  ) {
      ...
      <div className="flex min-h-screen items-center justify-center">
        <div className="rounded-lg bg-red-50 p-6 dark:bg-red-900/20">
          <p className="text-red-800 dark:text-red-200">
            Failed to load audit logs: {logsResult.error}
          </p>
        </div>
      </div>
    )
  ...
  if (!statsResult.success) {
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="rounded-lg bg-red-50 p-6 dark:bg-red-900/20">
          <p className="text-red-800 dark:text-red-200">
            Failed to load audit statistics: {statsResult.error}
          </p>
        </div>
      </div>
    )
      ...
      <div className="mb-8 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
          ...
          <p className="mt-1 text-2xl font-bold text-gray-600 dark:text-gray-400">
            {(stats.actionBreakdown.LOGIN || 0) +
              (stats.actionBreakdown.LOGOUT || 0) +
              (stats.actionBreakdown.EXPORT || 0) +
              (stats.actionBreakdown.APPROVE || 0) +
              (stats.actionBreakdown.DENY || 0) +
              (stats.actionBreakdown.CANCEL || 0)}
          </p>
        </div>
      </div>
