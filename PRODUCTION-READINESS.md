...
| 2   | ✅ **CLOSED IN CODE** | **Rate limiting now enforces without Redis.** The real defect was not the missing Upstash resource — it was that both limiter modules fell back to a mock that always returned success, so absent env vars silently disabled every limit in production. New `lib/rate-limit-fallback.ts` enforces a genuine in-process sliding window in production (permissive in development), covering all 11 limiters. 9 unit tests. Still **recommended**: provision Upstash for correct distributed limits — the fallback is per-instance, so a distributed attacker gets roughly N times the budget, and counters reset on instance recycle. Startup logs `[rate-limit] DEGRADED` while it is in use. |
...
**Recommended next, none of them blocking:** provision Upstash for distributed limits; reconcile
the migration history (`supabase migration repair`) and then apply
`20260728120000_revoke_complete_task_from_public.sql`; disable the legacy Supabase JWT key types;
purge the old key and credentials from git history; and land the fleet-timezone fix (P8).
