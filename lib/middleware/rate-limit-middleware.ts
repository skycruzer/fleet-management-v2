...
// default prefix is `@upstash/ratelimit`, so any instances that omit this
// option all share the same Redis key and the strictest limit wins — which
// is exactly how pilots on shared office IPs got locked out of /api/portal/login
// by normal portal traffic consuming the auth bucket.
// See: https://upstash.com/docs/redis/sdks/ratelimit-ts/features#using-multiple-limits
