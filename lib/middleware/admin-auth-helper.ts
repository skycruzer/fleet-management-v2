 ...
 *
 * Wrapped in React `cache()` so it resolves at most once per request. Several
 * callers (the route factory, `requireRole`, and handlers that want the admin's
 * identity) each used to trigger a fresh `auth.getUser()` plus an `an_users`
 * lookup, so a single admin API call could spend four network round trips
 * re-proving the same identity before reaching any business logic.
 */
export const getAuthenticatedAdmin = cache(
  async function getAuthenticatedAdmin(): Promise<AdminAuthResult> {
