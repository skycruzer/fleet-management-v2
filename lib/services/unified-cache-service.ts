...
class UnifiedCacheService {
  ...
  async getOrSet<T>(
    ...
    if (useRedis) {
      ...
      if (tags && tags.length > 0) {
        await redisCacheService.setWithTags(key, value, ttlSeconds, tags)
      } else {
  ...
  /**
   * Invalidate cache by tag (both local and Redis)
   */
  invalidateByTag(tag: string): void {
    ...
    // Also invalidate in Redis to prevent stale data on next read
    redisCacheService.invalidateByTag(tag).catch((err) => {
      logError(err as Error, {
        source: 'UnifiedCacheService',
        severity: ErrorSeverity.MEDIUM,
        metadata: { operation: 'invalidateByTag:redis', tag },
      })
    })
  }
