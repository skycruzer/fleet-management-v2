/**
 * Service: Renewal Planning Preview
 *
...
import { calculateRenewalWindow, PLANNABLE_CATEGORIES } from '@/lib/utils/grace-period-utils'
import {
...
export const VALID_CATEGORIES = [...PLANNABLE_CATEGORIES]

// ============================================================
// Type Definitions for Enhanced Preview Response
// ============================================================

export interface PairingPreviewItem {
  captain: {
    pilotId: string
    name: string
    employeeId: string
...
export interface UnpairedPreviewItem {
  pilotId: string
  name: string
  employeeId: string
...
export interface IndividualPreviewItem {
  pilotId: string
  name: string
  employeeId: string
  role: string
...
export interface RenewalPlanningPreviewParams {
  monthsAhead: number
  categories: string[]
  checkCodes?: string[]
  captainRoles?: string[]
...
export interface RenewalPlanningPreviewResult {
  totalPlans: number
  ...
  distribution: Array<{
    ...
    utilization: number
    byCategory: {
      'Flight Checks': number
      'Simulator Checks': number
      'Ground Courses Refresher': number
    }
    pairs: PairingPreviewItem[]
    unpaired: UnpairedPreviewItem[]
    individual: IndividualPreviewItem[]
...
const EMPTY_RESULT: RenewalPlanningPreviewResult = {
  totalPlans: 0,
  periodsAffected: 0,
  avgUtilization: 0,
  categoryBreakdown: {},
...
export async function previewRenewalPlan(
  params: RenewalPlanningPreviewParams
): Promise<RenewalPlanningPreviewResult> {
  const { monthsAhead, categories, checkCodes, captainRoles } = params
  ...
  const dbCategories = categories.filter((c) => VALID_CATEGORIES.includes(c as never))
  ...
  if (checkCodes && checkCodes.length > 0) {
    checkTypeIdsQuery = checkTypeIdsQuery.in('check_code', checkCodes)
  }
  ...
  for (const check of checks) {
    ...
    } else if (INDIVIDUAL_CATEGORIES.includes(category as never)) {
    ...
    const captainChecks =
      captainRoles && captainRoles.length > 0
        ? allCaptainChecks.filter((c) => {
            ...
            return captainRoles.includes(role)
          })
        : allCaptainChecks
  ...
  for (const [, preview] of rosterPeriodPreviews) {
    ...
    if (preview.totalCapacity > 0) {
      preview.overallUtilization = calculateUtilization(preview.totalPlanned, preview.totalCapacity)
    }
  }
