...
async function setupDatabase() {
  try {
    ...
    for (const period of rosterPeriods) {
      ...
      if (insertError) {
        if (insertError.message.includes('does not exist')) {
          ...
          console.log(
            `   1. Open Supabase SQL Editor: https://app.supabase.com/project/wgdmgvonqysflwdiiols/sql/new`
          )
    ...
    console.log(`\n   Last period:`)
    console.log(
      `      - ${capacityData[capacityData.length - 1].roster_period}: ${capacityData[capacityData.length - 1].period_start_date} to ${capacityData[capacityData.length - 1].period_end_date}`
    )
