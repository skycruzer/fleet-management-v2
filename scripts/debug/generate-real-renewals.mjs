...
async function generateRealPlans() {
  try {
    ...
    certs.slice(0, 5).forEach((cert) => {
      ...
      console.log(
        `      • ${pilot?.first_name} ${pilot?.last_name} (${pilot?.employee_id}) - ${checkType?.check_code} - Expires: ${cert.expiry_date}`
      )
    })
    console.log('')
    ...
    const insertResponse = await fetch(`${SUPABASE_URL}/rest/v1/certification_renewal_plans`, {
      method: 'POST',
      headers: {
        apikey: SUPABASE_KEY,
        Authorization: `Bearer ${SUPABASE_KEY}`,
