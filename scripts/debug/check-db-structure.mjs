...
if (data && data.length > 0) {
  ...
  data.forEach(req => {
    ...
    if (!summary[key]) {
      summary[key] = { roster_period: req.roster_period, request_type: req.request_type || 'NO_TYPE', count: 0, pending: 0, approved: 0 }
    }
    summary[key].count++
  ...
  Object.values(summary)
    ...
    .forEach(item => {
      console.log(`${item.roster_period.padEnd(12)} | ${item.request_type.padEnd(25)} | Total: ${item.count}, Pending: ${item.pending}, Approved: ${item.approved}`)
    })
} else {
