...
async function testPage(name, url, expectedElements = []) {
  ...
  try {
    ...
    if (hasError) {
      const errorText = await page.locator('text=/error|unauthorized|forbidden/i').first().textContent();
...
async function runTests() {
  try {
    ...
    await testPage(
      'Dashboard Home',
      `${BASE_URL}/dashboard`,
      ['text=/pilots/i', 'text=/certifications/i']
    );

    await testPage(
      'Pilots List',
      `${BASE_URL}/dashboard/pilots`,
      ['text=/pilot/i', 'text=/rank/i']
    );

    await testPage(
      'Certifications',
      `${BASE_URL}/dashboard/certifications`,
      ['text=/certification/i', 'text=/expiry/i']
    );

    await testPage(
      'Leave Requests',
      `${BASE_URL}/dashboard/leave-requests`,
      ['text=/leave/i', 'text=/request/i']
    );
