...
(async () => {
  ...
  try {
    ...
    const leaveBidLink = navLinks.find(link =>
      link.text.toLowerCase().includes('leave bid') ||
      link.href.includes('leave-bid')
    );
    ...
    console.log(`3. Leave Requests buttons: ${leaveRequestButtons.length > 0 ? '✅ FOUND' : '❌ NOT FOUND'}`);
