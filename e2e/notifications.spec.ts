...
test.describe('Admin Notifications', () => {
  ...
  test('should open notifications dropdown', async ({ page }) => {
    ...
    if (await notificationButton.first().isVisible()) {
      ...
      await expect(page.getByRole('menu').or(page.locator('[data-testid="notification-dropdown"]')).first()).toBeVisible({ timeout: 60000 })
    }
  })
})
...
test.describe('Pilot Portal Notifications', () => {
  ...
  test('should display notifications page', async ({ page }) => {
    await expect(page.getByRole('heading', { name: /notifications?/i })).toBeVisible({ timeout: 60000 })
  })

  test('should display notification list or empty state', async ({ page }) => {
    const hasNotifications = await page.locator('[data-testid="notification-item"]').first().isVisible().catch(() => false)
    const hasEmptyState = await page.getByText(/no notifications|all caught up/i).isVisible().catch(() => false)
  ...
  test('should mark notification as read', async ({ page }) => {
    ...
    if (await firstNotification.isVisible()) {
      ...
      const isRead = await firstNotification.locator('[data-read="true"]').isVisible().catch(() => false)
