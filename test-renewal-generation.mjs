...
async function testGeneration() {
  try {
    ...
    certs.slice(0, 5).forEach(cert => {
      ...
      console.log(`      • ${pilot?.first_name} ${pilot?.last_name} - ${checkType?.check_code} (${checkType?.category}) - Expires: ${cert.expiry_date}`);
    });
    console.log('');
    ...
    const { generateRenewalPlan } = await import('./lib/services/certification-renewal-planning-service.ts');
    ...
    try {
      ...
      capacities.forEach(cap => {
        ...
        const capacity = (cap.flight_capacity || 0) + (cap.simulator_capacity || 0) + (cap.ground_capacity || 0);
