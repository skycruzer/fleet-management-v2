...
-- DOCUMENTATION
-- ===================================
COMMENT ON FUNCTION submit_feedback_post_tx(uuid, text, text, uuid, boolean, text, text) IS 'Atomically creates a feedback post with validation. Returns success result or rolls back on error.';
COMMENT ON FUNCTION submit_leave_request_tx(uuid, text, date, date, integer, text, text) IS 'Atomically creates a leave request with pilot_id lookup and validation. Returns success result or rolls back on error.';
COMMENT ON FUNCTION submit_flight_request_tx(uuid, text, date, text, text) IS 'Atomically creates a flight request with pilot_id lookup and validation. Returns success result or rolls back on error.';
