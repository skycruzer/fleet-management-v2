...
-- DOCUMENTATION
-- ===================================
COMMENT ON FUNCTION delete_pilot_with_cascade IS 'Atomically deletes a pilot and all related records (leave requests, certifications). Returns summary of deletion.';
COMMENT ON FUNCTION batch_update_certifications IS 'Atomically updates multiple certifications. All updates succeed together or all fail together.';
COMMENT ON FUNCTION approve_leave_request IS 'Atomically approves/denies a leave request and creates an audit log entry. Both operations succeed together or fail together.';
COMMENT ON FUNCTION create_pilot_with_certifications IS 'Atomically creates a pilot and their initial certifications. Both pilot and all certifications are created together or none at all.';
COMMENT ON FUNCTION bulk_delete_certifications IS 'Atomically deletes multiple certifications. Useful for bulk cleanup operations.';
