...
async function executeSetup() {
  try {
    ...
    const { error: table3Error } = await supabase
      .from('renewal_plan_history')
      .select('id')
      .limit(1)
    ...
    for (const period of rosterPeriods) {
      ...
      if (insertError) {
        if (insertError.message.includes('does not exist')) {
          console.log(`   ⚠️  Table does not exist. Please run setup-renewal-planning.sql manually in Supabase SQL Editor.`)
          console.log(`\n📝 Instructions:`)
          console.log(`   1. Open Supabase Dashboard: https://app.supabase.com/project/wgdmgvonqysflwdiiols/sql/new`)
