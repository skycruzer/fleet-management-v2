...
async function testLogin() {
  try {
    ...
    if (error) {
      ...
      const adminSupabase = createClient(SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY || SUPABASE_ANON_KEY);
    ...
    for (const test of apiTests) {
      try {
        ...
        if (response.ok) {
          ...
          console.log(`  ✅ ${test.name}: ${response.status} (${json.data?.length || 'N/A'} records)`);
        } else {
